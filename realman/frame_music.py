...
received_signal_time = received_signal_time.squeeze(1).numpy()

frame_size = 1024
hop_size = 256

for i in range(0, len(received_signal_time[0]) - frame_size, hop_size):
    clip = received_signal_time[:, i:i+frame_size]

    # 时域数据预处理：FFT转换到频域
    clip = fft(clip, axis=1)
    freq_bins = np.fft.fftfreq(num_samples, 1 / sampling_rate)
    freq_idx = np.argmin(np.abs(freq_bins - frequencies[0]))  # 选择信号频率对应的频点
    received_signal = clip[:, freq_idx].reshape(-1, 1)  # 提取频域数据
    ...
    plt.grid(True)
    # plt.show()
    plt.savefig("/home/ubuntu/project/DOA/手撕代码/result/" + str(i) + ".png")
    plt.close()