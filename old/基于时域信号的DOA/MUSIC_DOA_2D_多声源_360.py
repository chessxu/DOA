...
def compute_covariance_matrix(signals):
    ...
    # 可以尝试对协方差矩阵进行正则化，减少噪声的影响
    R += 0.01 * np.eye(num_mics)  # 添加微小的正则化项（避免矩阵奇异）

    return R


# 4. MUSIC算法定位多声源
def music_algorithm(array_geometry, covariance_matrix, num_sources, fs, angles=np.linspace(-180, 180, 361)):
    """基于MUSIC算法估计多个声源的DOA"""
    # 计算协方差矩阵的特征值和特征向量
...
if __name__ == "__main__":
    ...
    # 设定声源位置 (假设为真实位置)
    sources = [
        {"position": np.array([1.0, 0.5]), "signal": np.random.randn(int(1.0 * fs))},  # 声源1的位置
        {"position": np.array([-0.5, -0.8]), "signal": np.random.randn(int(2.0 * fs))},  # 声源2的位置
    ]
    signals, array_geometry = generate_data_2D.create_simulate_data(sources)  # 使用你自己的生成仿真数据的函数
    ...
    # 可视化MUSIC谱
    plot_music_spectrum(spectrum, np.linspace(-180, 180, 361), estimated_angles)
